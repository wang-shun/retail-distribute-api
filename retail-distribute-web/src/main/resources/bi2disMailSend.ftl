...
<body>
			...
			<#if pros??>
				<#list pros?keys as key>
					...
					<#if pro.success == false>
						<span style='color:red;padding-left:20px'>货号:${pro.productCode}同步失败,同步时间:${pro.updateTime},错误信息:${pro.errorInfo},同步的尺寸:${pro.sizeCodeStr},未同步的尺寸:${pro.unSynSizeCodeStr}</span>
					<#else>
						<span style='color:green;padding-left:20px'>货号:${pro.productCode}同步成功,同步时间:${pro.updateTime},同步的尺寸:${pro.sizeCodeStr},未同步的尺寸:${pro.unSynSizeCodeStr}</span>
					</#if>
					<#if pro.warnInfo??>
						<br/><span style='color:blue;padding-left:20px'>警告信息:${pro.warnInfo}</span>
					</#if>
					<br/>
				</#list>
			</#if>
	...
	<#else>
				<h3>本次没有信息需要同步,时间${synTime?datetime}</h3>
	</#if>
</body>
</html>