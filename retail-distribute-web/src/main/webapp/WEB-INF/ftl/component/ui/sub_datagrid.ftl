...
<#macro subdatagrid
	...
	emptyMsg="暂无数据" showFooter="false" border="false" subGridColumnsJsonList="[]" loadSubGridUrl="" 
	subPagination="false" subHeight='200' subSingleSelect='false' detailNo="billNo">
    ...
    $(function() {
        ...
        var $dgExtend={
	        ...
	        <#if onClickRowAuto='true'> 
		    	...
		    	subColumns : [${subGridColumnsJsonList}],
		     <#if loadSubGridUrl !=''> 	
		     	view: detailview,
	            detailFormatter:function(index,row){
	                return '<div style="padding:2px;height:${subHeight}px"><table class="ddv" id="ddv-' + index + '"></table></div>';
	            },
	            onExpandRow: function(index,row){
	           		detailIndex = index;
	                var ddv = $(this).datagrid('getRowDetail',index).find('table.ddv');
	                var url = "<@s.url loadSubGridUrl/>";
	                <#if detailNo!=''>
	                	url += "?${detailNo}=" + row.${detailNo};
	                </#if>
	                ddv.datagrid({
	                	<#if subSingleSelect='true'>
			                singleSelect:true,
			                <#else>
			                singleSelect:false,
			            </#if>
	                    ...
	                    columns:[${subGridColumnsJsonList}],
	                    onResize:function(){
